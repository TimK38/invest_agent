...
def main():
    ...
    print(f"    2. 標的池 {len(full)} 檔，仍偏重半導體/面板 + 高股息 ETF，產業分散不足。")
    print(f"    3. A 組的穩健比重做到 {a['穩健比']:.0%}（目標 72%），"
          f"平均曝險 {a['曝險']:.2f} vs B/C 的 {b['曝險']:.2f}。")
    print(f"       補進 7 檔低風險係數標的之後這個比較已大致公平，"
          f"但 A 仍受單一標的上限壓制而配不滿 72%。")
    print("    4. **這個樣本對穩健核心先天不利**：+152% 的大多頭裡，"
          "強迫 63% 資金放在低 Beta 標的")
    print("       （0056 +52%、00878 +67%、00919 +49%，遠輸大盤 +154%）"
          "本來就會拖累報酬。")
    print("       穩健核心防的是**空頭**，而這三年沒有空頭。"
          "A 的最大回撤確實較小（見上表），只是在這個樣本裡不划算。")
    print("    5. 模擬用收盤價、每日再平衡、無跳空與滑價。實際執行做不到這麼準。")
    print()
